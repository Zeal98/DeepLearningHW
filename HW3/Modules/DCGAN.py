...
class Discriminator(nn.Module):
    def __init__(self, save_path='epoch_dcnetg.pth'):
...
class Generator(nn.Module):
    def __init__(self, save_path='epoch_dcnetd.pth'):
...
def train(dataloader, netg_path, netd_path, num_epochs=50):
    ...
    for epoch in range(num_epochs):
        ...
        if ((epoch + 1) % 10 == 0):
            t.save(netd.state_dict(), 'Images/DCGAN/dc_d%s' % epoch)
            t.save(netg.state_dict(), 'Images/DCGAN/dc_g%s' % epoch)
        save_image(fake_u.data[:32], "Images/DCGAN/%d.png" % epoch, nrow=8, normalize=True)
